...
fluence is a CLI tool for managing Confluence pages and spaces from the terminal. It provides bidirectional Markdown conversion (Markdown ↔ Confluence storage format), enabling a local-first documentation workflow.
...
go build -ldflags "-X main.version=v1.0.0" -o fluence
...
```bash
./fluence space list
./fluence page list -s MYSPACE
echo "# Test" | ./fluence page create -t "Test Page"
...
```bash
./fluence space view YOUR_SPACE_KEY
```
