...
This directory contains test fixtures for validating fluence's bidirectional Markdown ↔ Confluence conversion.
...
cat testdata/comprehensive-test.md | ./fluence page create -t "Test Page" --parent PAGE_ID
...
These are limitations of Confluence itself that cannot be fixed in fluence.
...
cat testdata/comprehensive-test.md | ./fluence page create -t "Comprehensive Test" --parent PARENT_ID

# View the page content as Markdown
./fluence page view PAGE_ID

# View raw storage format
./fluence page view PAGE_ID -j | jq -r '.body.storage.value'

# Update and verify changes persist
cat testdata/comprehensive-test.md | ./fluence page update PAGE_ID
./fluence page view PAGE_ID
```
...
```bash
# Test just tables
echo '| A | B |\n|---|---|\n| 1 | 2 |' | ./fluence page create -t "Table Test"

# Test just task lists
echo '- [ ] Todo\n- [x] Done' | ./fluence page create -t "Task Test"

# Test code blocks with special characters
echo '```go\nregexp.MustCompile(`^[A-Z]:\\\\[\\w\\\\]+$`)\n```' | ./fluence page create -t "Code Test"
```
