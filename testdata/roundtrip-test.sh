#!/bin/bash
#
# Round-trip test for fluence Markdown conversion
#
...
FLUENCE_BIN="$PROJECT_DIR/fluence"
...
# Build fluence if needed
log_step "Building fluence"
cd "$PROJECT_DIR"
if [[ ! -f "$FLUENCE_BIN" ]] || [[ "main.go" -nt "$FLUENCE_BIN" ]] || [[ -n "$(find cmd internal -name '*.go' -newer "$FLUENCE_BIN" 2>/dev/null)" ]]; then
...
# Verify fluence works
FLUENCE_VERSION=$("$FLUENCE_BIN" --version 2>&1 || echo "unknown")
log_info "fluence version: $FLUENCE_VERSION"

# Create the test page
log_step "Creating test page in Confluence"
log_info "Uploading comprehensive-test.md..."

CREATE_OUTPUT=$(cat "$TEST_FILE" | "$FLUENCE_BIN" page create -t "$PAGE_TITLE" --parent "$PARENT_ID" 2>&1)
...
# Get the content (skip the header lines from fluence page view)
"$FLUENCE_BIN" page view "$PAGE_ID" | tail -n +6 > "$RETRIEVED_FILE"
